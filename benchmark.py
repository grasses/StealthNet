...
class ModelWrapper:
    ...
    def torch_model(self):
        ...
        ckpt = torch.load(os.path.join(self.torch_model_path, 'final_ckpt.pth'), map_location=sys_args.device)
    ...
    def load_saved_weights(self, torch_model):
        ...
        if os.path.exists(ckpt_path):
            ckpt = torch.load(ckpt_path, map_location=sys_args.device)
            torch_model.load_state_dict(ckpt['state_dict'])
...
class ImageBenchmark:
    def __init__(self, datasets_dir='data', models_dir='model/cpkt'):
